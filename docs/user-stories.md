...
| US-THEME-01 | Als Nutzer wähle ich das Marken-Theme (hestia oder das Design System der Schweizer Bundesverwaltung) und die Ansicht (automatisch/hell/dunkel) über einen JS-freien, server-seitig gespeicherten Umschalter; die Oberfläche übernimmt Farben und Schrift des gewählten Themes. | Tokens + go/core (A) | 10 | aktiv | `e2e/theme-switch.spec.ts` (+ `e2e/tokens.spec.ts`) |
